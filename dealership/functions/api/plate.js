/*
  ...
  Cloudflare runs this automatically for any request to /api/plate, right
  alongside the static index.html. It calls two free government services and
  merges them, because neither one alone knows enough:
  ...
  Setup — DVSA MOT history (free):
    1. Register at https://documentation.history.mot.api.gov.uk
       You are emailed a client ID, a client secret and an API key.
    2. Add three more secrets:
         MOT_CLIENT_ID
         MOT_CLIENT_SECRET
         MOT_API_KEY
       (and MOT_TENANT_ID only if DVSA gave you a tenant other than the default)

  Then redeploy. With neither configured the endpoint answers 501 and the app
  falls back to reading the registration period off the plate itself.
*/
