/*
  Number plate lookup — Netlify function serving /api/plate.
  ...
  Keys live in the site's environment variables — Netlify dashboard, then
  Site configuration -> Environment variables. Add them, then redeploy:
...
async function handlePlate(request, env) {
...
/*
  Netlify Functions v2 entry point. The config below claims the /api/plate URL
  directly, so no redirect rules are needed. Keys come from the site's
  environment variables, set in Site configuration -> Environment variables.
*/
export default async (request) => handlePlate(request, process.env);
